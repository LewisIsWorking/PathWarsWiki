# PW~Multi-discord-ad

Ping: @PF2e LFG

Hi!
I am running a couple of Play-by-Posts, and I am looking for players to fill some slots.
- This is NOT a paid game.
- This is NOT a west march.
- You can join any single campaign or multiple campaigns, if you wish.

For all of these campaigns:
**System: ** Pathfinder 2e.
**Open Slots: ** 6.
**Pace: ** Slow to medium, typically 1-2 posts per week. Can be more active, just depends on players and my availability.
**Post Length: ** Any.
**Content Warning: ** Please read the content warning guide in the #join-the-campaign channel of the Discord.
**Requirements: ** None.
**Nice-to-Haves: ** A computer.
**Time & Frequency: ** Over text chat, at your own pace.
**Start: ** N/A.
**Platforms: ** Foundry VTT, Discord & Telegram. These apps are all free for players to use. Happy to help anyone get set up. :slight_smile:

There are slots open in all of these campaigns. 

~~__01 Riddleport__
**Campaign Description: **
- Pirate-themed campaign set in Riddleport, a city of scoundrels and rogues.
- The party will be involved in various adventures, including treasure hunting, dealing with pirates and uncovering the secrets of the city.
- Level 2 → 20.~~
Full!

__04-The-Magni-Guard-The-Elite-Guards-Of-Magnimar__
**Campaign Description: **
- Set in Magnimar, the party will take on the roles of agents of Magni-Guard, dealing with various threats to the city.
- The campaign will involve investigations, combat and exploration of the city and its surroundings.
- (This is the agents of Edge watch, but set in Magnimar instead of Absalom).
- Level 1 → 20.

__05-The-Grand-Explorers__
**Campaign Description: **
- The party will take on the roles of the Grand Explorers, a group dedicated to exploring the world and
  uncovering its secrets.
- The campaign will involve exploration, combat and interaction with various factions and cultures.
- Level 1 → 20.

__06-Kibwe__
**Campaign Description: **
- Save the city of Kibwe from a magical curse that is turning its citizens into slime-people.
- Level 7 → 8.

__07-The-Hopeful-Endtimes__
**Campaign Description:**
- System: StarFinder 2e.
- We are playing through some StarFinder 2e PT content.
- Level 2 → 15.

~~__09-Metal-City-Stargazing.__
**Campaign Description:**
- System: StarFinder 2e.
- Metal City adventure module.
- Level 1.~~
- Full!

**How to join: **
- Join the Discord server, use the link in my bio.
- Then go to the #join-the-campaign channel and react with the ✅ emoji to gain access to the rest of the server.