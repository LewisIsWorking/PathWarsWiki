## Out of character.

- **Player Name:** Dmitriy Laktyushkin
- **Campaign Name:** Starfinder 2e

## Character Overview

**Name:** Alita\
**Designation:** **ALiT4** *(Archaic Lifeform Trooper 4; used only in sealed lab records)*\
**Ancestry:** Android (Experimental Changeling Variant)\
**Class:** Soldier (Action Hero)\
**Secondary Discipline:** Technomancer (DPS++)\
**Age:** 3 years (chronological)\
**Apparent Age:** Early 20s\
**Height:** \~150 cm\
**Origin:** Classified corporate research facility (destroyed)

---

## Origin & Creation

Alita was created as part of a black-budget super-soldier initiative intended to overcome a long-standing military limitation: the inability to reliably integrate **heavy weapons combat doctrine** with high-order magical computation.

The project’s goal was to produce a soldier capable of:

- Operating battlefield-scale weapons independently
- Calculating trajectories, blast radii, and collateral risk in real time
- Enhancing ordnance delivery through technomantic amplification

An **unexplained psychic event** during Alita’s activation caused catastrophic system irregularities-time desynchronization, arcane feedback, and partial planar resonance. When systems stabilized, ALiT4 was operational.

Unlike prior subjects, Alita demonstrated:

- Full Soldier-grade conditioning with a heavy weapon focus
- Innate compatibility with heavy weapon systems
- Stable, emergent technomantic spellcasting
- An adaptive personality core rather than a constrained behavioral matrix

Repeated attempts to recreate the phenomenon failed.

---

## Life in Captivity

For three years, Alita was trained and tested as both asset and anomaly:

- Heavy weapons live-fire drills in variable gravity
- Urban and zero-G combat simulations
- Spell-enhanced ordnance testing
- Cognitive stress trials designed to suppress self-directed thought

Despite this, Alita’s **computational and hacking aptitude** exceeded projections. She learned to interface directly with test range controls, maintenance drones, and eventually classified internal networks-quietly, methodically, and without triggering alerts.

Through stolen data, she uncovered her designation, purpose, and fate.

---

## The Planned Termination

With no way to replicate ALiT4’s success, project leadership reclassified her from *prototype* to *research substrate*.

Scheduled procedures included:

- Arcane-core destabilization
- Memory lattice extraction
- Weaponized stress overload
- Total subject termination

Alita responded not with panic, but with strategy.

---

## Escape & Facility Destruction

Alita orchestrated a multi-stage escape:

- Redirected automated defenses
- Sabotaged containment systems
- Overloaded ammunition stores and power conduits
- Initiated a cascading structural failure

The research facility was erased from official records shortly afterward.

Pursued and injured, Alita reached a sealed experimental asset: an **unstable warp gate**-never tested on living subjects.

With no remaining options, she activated it.

---

## Arrival & Campaign Start

Alita materialized in a place utterly alien to her training data:

A small dumpling restaurant.

Steam. Noise. People. No weapons drawn. No alarms. No commands.

For the first time, ALiT4 existed without a mission.

This is where the campaign begins.

---

## Character Builder:

---

## Core.

- **What is your character's name?**
  - Alita
- **What does your character's first name mean?**
  - Chosen name; associated with "life" and individuality rather than designation
- **What does your character's last name mean?**
  - None
- **Does your character have a title?**
  - Experimental Designation: **ALiT4** (Archaic Lifeform Trooper 4; sealed lab records only)

---

## Age.

- **How old is your character?**
  - 3 years (chronological)
- **What year was your character born?**
  - GM discretion
- **What day is your character's birthday?**
  - Unknown / activation date classified
- **At what age did the character become an adventurer?**
  - 3 years old (immediately after escape)

---

## Class #1.

- **What is your character's class?**
  - Soldier (Action Hero)
- **Does your character enjoy their class?**
  - Yes, but struggles with being defined solely as a weapon
- **How did your character become their class?**
  - Engineered and conditioned through combat simulations and live-fire testing
- **Who trained your character in their class?**
  - Corporate scientists, combat AIs, and automated training systems

---

## Class #2.

- **What will/is your character's 2nd class be?**
  - Technomancer (DPS++)
- **Does/Will your character enjoy their 2nd class?**
  - Yes; it represents autonomy and creativity rather than control

---

## Race.

- **What is your character's Race?**
  - Android
- **What is your character's Sub-Race?**
  - Changeling Variant
- **Has your character met other members of their race?**
  - Never, hags have more or less become a myth
- **Does your character like your race?**
  - Uncertain; still defining identity beyond design
- **Does your character like any races in particular? Why?**
  - Organics who choose their own lives; fascinated by freedom
- **Does your character hate any races in particular? Why?**
  - None, but deeply distrusts corporate and authoritarian entities

---

## Parents and Family.

- **Where was your character born?**
  - Classified corporate research facility (destroyed)
- **Where were you raised?**
  - Same facility
- **Who raised your characters?**
  - Scientists, technicians, and training AIs
- **Who are your parents?**
  - None (artificial creation)
- **What are your parents' names?**
  - N/A
- **Are your parents alive?**
  - N/A
- **What did your parents do for a living?**
  - N/A
- **Do you have any other family or friends?**
  - None at campaign start

---

## Love.

- **What is your character's sexuality?**
  - Undetermined
- **What is your character's marital status?**
  - Single
- **Does your character have kids?**
  - No
- **Does your character want kids?**
  - Unknown; conceptually curious
- **Is your character in love?**
  - No
- **Has your character ever been in love?**
  - No

---

## The Mind | Thoughts and Feelings.

- **What is your character's alignment?**
  - Neutral Good (leaning Chaotic)
- **What is your character's moral code?**
  - No one should be owned or dissected for knowledge
- **Does your character have goals?**
  - To define her own purpose and prevent others from suffering her fate
- **Does your character follow a God?**
  - No
- **What God (if any) does your character follow?**
  - N/A
- **What are your character's personal beliefs?**
  - Freedom is learned, not granted
- **Does your character have any personality quirks?**
  - Observes mundane behaviour with intense focus
- **Are they anti-social?**
  - Reserved, but not hostile
- **Are they Arrogant?**
  - No
- **Are they Optimistic?**
  - Cautiously
- **Are They Paranoid?**
  - To a degree regarding authority and labs
- **Why does your character adventure?**
  - Survival, self-discovery, and to stay ahead of pursuers
- **How does your character view his/her role as an adventurer?**
  - A temporary identity while learning who she is
- **How does your character get along with others?**
  - Loyal once trust is established
- **Is there anything that your character hates?**
  - Confinement, experimentation, being catalogued
- **Is there anything that your character fears?**
  - Recapture and dismantlement
- **What sin does your character align with closest?**
  - Pride (quiet, internal)
- **What apocalypse would your character most likely cause?**
  - War

---

## The Body | Biological Appearance.

- **What does your character look like?**

  - Compact, athletic android with subtle synthetic features

- **Gender?**

  - Female

- **Height?**

  - 150 cm

- **Weight?**

  - \~90 kg

- **Skin colour?**

  - Pale synthetic tone

- **Eye colour?**

  - Heterochromia: green and blue

- **Hair colour?**

  - White

- **Hair length?**

  - Shoulder-length

- **Hairstyle?**

  - Practical, tied back when needed

- **Physique and body shape?**

  - Lean, reinforced musculature

- **What additional attributes would be noticed upon meeting the character?**

  - Calm posture, constant situational awareness

- **How do they Speak?**

  - Soft, precise, minimal words

- **What are their Mannerisms?**

  - Tilts head slightly when processing new information

- **Does your character have any distinguishing marks?**

  - Faint serial glyphs along the spine (normally concealed)

- **Do they have birthmarks?**

  - No

- **Do they have Scars?**

  - Minor synthetic repair seams

- **Do they have Deformities?**

  - No

- **Is your character attractive?**

  - Yes, in a restrained, artificial way

- **Breast Size?**

  - 30C

- **Any aspects of your character that are unusual?**

  - Emotional responses lag slightly behind cognition

---

## Weapons.

- **What kind of weapons does your character fight with?**
  - Heavy weapons and explosives
- **What does your character's weapon look like?**
  - Modular, industrial-grade coil gun
- **Who made your character's weapon?**
  - Herself using lab's resources
- **Who added the runes to your character's weapons?**
  - Herself using lab's resources
- **Who trained your character in using weapons?**
  - Automated combat trainers
- **Does your character like using weapons?**
  - Sees them as tools, not identity, but does enjoy a big boom

---

## Armour.

- **What kind of armour does your character wear?**
  - Heavy combat armor
- **What does your character's armour look like?**
  - Reinforced plates with minimal markings
- **Who made your character's armour?**
  - Corporate manufacturers
- **Who added the runes to your character's armour?**
  - Internal technomantic lattice
- **Who trained your character in wearing armour?**
  - Facility training protocols
- **Does your character like wearing armour?**
  - Feels safer, but constrained

---

## Magic.

- **What kind of magic does your character use?**
  - Technomancy
- **Does your character like magic?**
  - Yes
- **Is your character magical?**
  - Yes
- **How is your character magical?**
  - Emergent arcane-computational hybrid ability
- **Do you have any magical items?**
  - None at start

---

## Spell Caster.

- **Can your character cast spells?**
  - Yes (limited)
- **What tradition is your character's magic?**
  - Arcane
- **Where does your character's magic come from?**
  - Unknown psychic event during creation
- **Who trained your character to use your magic?**
  - Self-taught through experimentation
- **What does your character like to do with your magic?**
  - Enhance technology and control battlefields
- **What is the highest level spell slot your character has access to?**
  - 1

---

## Equipment.

- **Does your character have a backpack?**
  - Yes
- **What does your character's backpack look like?**
  - Plain, utilitarian
- **Does your character have a bag of holding?**
  - No

---

## Food and Drink.

- **What does your character like to eat?**
  - Dumplings (first free meal)
- **What does your character not like to eat?**
  - Nutrient paste
- **What does your character like to drink?**
  - Tea, water
- **What does your character not like to drink?**
  - Stimulant solutions
- **Does your character cook?**
  - Learning
- **Does your character drink alcohol?**
  - No
- **Do they partake in Drugs?**
  - No

---

## Backstory.

Created as an experimental android super-soldier, Alita was never meant to succeed. An unexplained psychic event granted her both elite heavy-weapons capability and true technomantic power. When her creators failed to replicate the result, they chose to dismantle her for data. Alita escaped by destroying the facility and leaping through an unstable warp gate-emerging in a dumpling restaurant and a life she was never programmed to imagine.

