# C01 Doomsday Funtime - 2024-01

> ⚠️ **Generated page - do not edit here.**
> This transcript is archived automatically by the PathWarsNudge bot
> and copied into the wiki. Any change made on this page is lost on
> the next sync. Fix it in the bot's `data/pbp_logs/` instead.
{#generated-banner}

*PBP transcript archived by PathWarsNudge bot.*

---

**Path Wars** *(GM)* · 01:09
## Session 202 = 2024 January 13th Saturday + 4710 AR Erastus 28th + Gaunt-light part 11.

**Path Wars** *(GM)* · 01:13
Following Selenor, the party slid down the rubbish chute, finding themselves in a vast slimy cavern.

**Path Wars** *(GM)* · 01:13
The party are in exploration mode.

**Path Wars** *(GM)* · 01:48
— What now?

**None** · 17:24
How dark is it?

**None** · 23:36
Perception to see how relatively vast the cavern is and if I can identify the slime / source of the slime.

**None** · 23:36
And if there is a bunch of rubbish down here or where it might be going from here?
